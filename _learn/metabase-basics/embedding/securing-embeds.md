...
image: /learn/images/securing-embeds/interactive-embedding-sso.png
author: The Metabase Team
redirect_from:
  - /learn/customer-facing-analytics/securing-embeds
---
...
1. **Authentication** looks at _who_ someone is (using standards such as [JWT](/docs/latest/people-and-groups/authenticating-with-jwt) or [SAML](/docs/latest/people-and-groups/authenticating-with-saml)).
2. **Authorization** looks at _what_ someone has access to (using standards such as OAuth 2.0).

## Public embedding

[Public embedding](/docs/latest/embedding/public-links#public-embeds) doesn't involve any authentication or authorization. A public embed displays a public link with a unique string at the end, like this:
...
To apply and hide the "Status = Active" filter, we'll add [query parameters](/docs/latest/embedding/public-links#public-embed-parameters) to the end of the public link in our embed:
...
- Metabase [permissions](/docs/latest/permissions/introduction) and [data sandboxes](/docs/latest/permissions/data-sandboxes) won't work --- if you need to lock down sensitive data, you must set up [locked parameters](#example-securing-data-with-locked-parameters-on-a-static-embed) for _each_ of your static embeds.
- Any filter selections in a static embed will reset once the signed JWT expires.
- All Static embed usage will show up in [usage analytics](/docs/latest/usage-and-performance-tools/usage-analytics) under "External user".
...
![Static embedding with JWT authorization.](../../images/securing-embeds/signed-embedding-jwt.png)

This diagram illustrates how an embed gets secured by a signed JWT:

1. **Visitor arrives**: your frontend gets a request to display a Metabase [embedding URL](/docs/latest/embedding/static-embedding#adding-the-embedding-url-to-your-website).
2. **Signed request**: your backend generates a Metabase embedding URL with a [signed JWT](/docs/latest/embedding/static-embedding#how-static-embedding-works). The signed JWT should encode any query [parameters](/docs/latest/embedding/static-embedding-parameters) you're using to filter your data.
...
- An [embeddable dashboard](/docs/latest/embedding/static-embedding#making-a-question-or-dashboard-embeddable) with the Accounts data.
- A [locked parameter](/docs/latest/embedding/static-embedding-parameters) for the Account ID filter.
...
3. Our app backend uses Metabase's [secret key](/docs/latest/embedding/static-embedding#regenerating-the-static-embedding-secret-key) to [generate the embedding URL](/docs/latest/embedding/static-embedding#how-static-embedding-works) with a signed JWT. The signed JWT encodes the query parameters to filter the Accounts dashboard on `Account ID = account_id`.
...
- [Tables](/docs/latest/permissions/data)
- [Rows](/learn/metabase-basics/administration/permissions/data-sandboxing-row-permissions)
- [Columns](/learn/metabase-basics/administration/permissions/data-sandboxing-column-permissions)
- [Other data permissions](/docs/latest/permissions/data), such as data download permissions or SQL access.

### Interactive embedding with SSO

![Interactive embedding with SSO.](../../images/securing-embeds/full-app-embedding-sso.png)

This diagram shows you how a interactive embed gets secured with [SSO](/docs/latest/people-and-groups/start#authentication):
...
2. **Load embed**: your frontend component loads the Metabase frontend using your [embedding URL](/docs/latest/embedding/interactive-embedding#pointing-an-iframe-to-a-metabase-url).
...
4. **If there's no valid session**:
   ...
   - **SSO auth**: your SSO flow authenticates the visitor and generates a session based on their identity. The session info should encode user attributes such as group membership and [data sandboxing](/docs/latest/permissions/data-sandboxes) permissions.
...
The mechanics of step 4 will vary a bit depending on whether you use [JWT](/docs/latest/people-and-groups/authenticating-with-jwt) or [SAML](/docs/latest/people-and-groups/authenticating-with-saml) for SSO.
...
The nice thing about interactive embedding and [SSO](/docs/latest/people-and-groups/start#authentication) integration is that we don't have to manually manage locked parameters for each embed. Instead, we can map user attributes from our identity provider (IdP) to [permissions](/docs/latest/permissions/introduction) and [data sandboxes](/docs/latest/permissions/data-sandboxes) in Metabase. People can get authenticated and authorized to self-serve specific subsets of data from their very first login.
