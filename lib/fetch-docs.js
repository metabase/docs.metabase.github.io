...
/**
 * Downloads, extracts, and updates the documentation for a specified Metabase version.
 *
 * Depending on the `settings`, it either:
 * - Updates the existing documentation by reading the version from `_config.yml`, or
 * - Sets the major version by squashing a full version (e.g., `v0.41.1` → `v0.41`).
 *
 * Then it:
 * - Updates version metadata
 * - Downloads the appropriate Metabase tarball from GitHub
 * - Extracts the tarball to a temporary directory
 * - Moves the documentation files into their final destination
 *
 *
 * @async
 * @function fetchDocs
 * @param {Object} settings - Configuration object for the documentation fetch.
 * @param {boolean} [settings.update] - If true, updates existing docs based on `_config.yml`.
 * @param {string} [settings.config] - Path to `_config.yml`, used if `update` is true.
 * @param {string} [settings.setVersion] - Explicit version to set.
 * @param {string} [settings.version] - Full version string (e.g., `v0.41.1`).
 * @param {string} [settings.finalDir] - Path to final destination for docs.
 * @returns {Promise<void>} Resolves when documentation is fully updated and moved.
 */
...
function extractMetabaseRepoTarball(filename, settings) {
  console.log("extractMetabaseRepoTarball:", filename, settings);
  return new Promise((resolve) => {
    ...
    extract.on("entry", function extracting(header) {
      ...
      console.log("Creating dir for ", settings.tmpDir, extractedElectronDir);
      console.log("Path: ", path.join(settings.tmpDir, extractedElectronDir));
      settings.newDir = path.join(settings.tmpDir, extractedElectronDir);
    });

    extract.on("finish", function() {
      console.log("extractMetabaseRepoTarball: done.").
      resolve();
    });
...
async function generateEmbeddingSdkApiDocs(metabaseProjectRoot) {
  ...
  await runCommand('yarn', ['install'], options);
  await runCommand('yarn', ['embedding-sdk:docs:generate'], options);
