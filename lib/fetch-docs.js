...
module.exports = async function fetchDocs(settings) {
  ...
  if (settings.repoDir) {
    // Run doc generation against the real local checkout (it needs the full
    // repo: source files, lockfile, node_modules), then copy the resulting
    // docs/ into a scratch dir. Everything downstream (frontmatter rewriting,
    // directory renames) operates on that scratch copy, not the user's repo.
    console.log("Generating docs for the Embedding SDK API in, ", settings.repoDir);
    await generateEmbeddingSdkApiDocs(settings.repoDir);
    await generateModularEmbeddingApiDocs(settings.repoDir);
    ...
    await generateModularEmbeddingApiDocs(settings.newDir);
  }
...
async function generateModularEmbeddingApiDocs(metabaseProjectRoot) {
  const packageJsonPath = path.join(metabaseProjectRoot, "package.json");

  const packageJson = JSON.parse(fs.readFileSync(packageJsonPath, "utf-8"));
  const hasDocsScript = packageJson.scripts["embedding-eajs:docs:generate"];

  if (!hasDocsScript) {
    return;
  }

  console.log("Generating Modular Embedding API docs...");
  ...
  const usesBun =
    packageJson.packageManager && packageJson.packageManager.startsWith("bun");
  const pm = usesBun ? "bun" : "yarn";
  const runArgs = usesBun
    ? ["run", "embedding-eajs:docs:generate"]
  ...
  console.log("Modular Embedding API docs generated!");
}
