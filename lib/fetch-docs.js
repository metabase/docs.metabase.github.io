...
module.exports = async function fetchDocs(settings) {
  ...
  if (settings.newDir !== undefined && fs.existsSync(settings.newDir)) {
    console.log(
      `'${settings.newDir}': Directory set and exists. Skipping download.`,
    );
  } else {
    await downloadAndExtractMetabaseTarball(settings);
  }

  // from here on, settings.newDir is the directory where the metabase repo exists
  // whether we downloaded it or the directory already existed.
...
async function moveDirectories(settings) {
  const ogSettings = {... settings};
  console.log("settings inside move Directories: ", settings);
  settings.versionDir = path.join(settings.newDir, settings.majorVersion);
  console.log("settings.versionDir: ", settings.versionDir);
  settings.extractedDocsDir = path.join(settings.newDir, "docs");
  console.log("settings.extractedDocsDir: ", settings.extractedDocsDir);
  settings.finalDir = path.join(
    process.cwd(),
    settings.finalDir,
    settings.majorVersion,
  );
  console.log("settings.finalDir: ", settings.finalDir);
  console.log("Renaming ", settings.extractedDocsDir, "to:", settings.versionDir)
  await fs.promises.rename(settings.extractedDocsDir, settings.versionDir);

  console.log("cpr-ing", settings.versionDir, "to:", settings.finalDir);
  ...
  console.log("OG Settings: ", ogSettings);

  console.log("rimraf-ing: ", settings.tmpDir);
