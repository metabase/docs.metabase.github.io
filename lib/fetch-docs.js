...
async function downloadAndExtractMetabaseTarball(settings) {
  const tarVersion = settings.branchName ? settings.branchName : settings.version;
...
async function moveDirectories(settings) {
  ...
  console.log("extractedDocsDir: ", extractedDocsDir);
  ...
  // tmpDir will not be set if we clone the repo instead of downloading a
  // tarball: (--repo-dir option passed into script/docs)
  if (settings.tmpDir) {
    console.log("rimraf-ing: ", settings.tmpDir);
    await rimraf(settings.tmpDir || '');
  } else {
    console.log("skipping rimraf of tmpDir, not set");
  }
