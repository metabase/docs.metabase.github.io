...
3. Run a query against your database from the Metabase SQL editor to check for database connection or database privilege errors that aren't listed in the logs:
    ...
    LIMIT 1
    ```
5. [Manually re-sync](../databases/connecting#manually-syncing-tables-and-columns) the table or view if needed.
...
- be [granted privileges](./data-permissions#granting-database-privileges) to query that database. 
...
To speed up **syncs**:
   ...
   - [Reduce the frequency of sync queries](../databases/connecting#scheduling-database-scans).

To speed up **scans**:
   - [Reduce the frequency of scans, or disable scans entirely](../databases/connecting#scheduling-database-scans).
...
- [How syncs and scans work](../databases/connecting#syncing-and-scanning-databases).
