...
The databases listed below have official drivers maintained by the Metabase team. Customers on [paid plans](/pricing) will get official support.

- [Amazon Athena](./connections/athena)
- [BigQuery](./connections/bigquery) (Google Cloud Platform)
- [Druid](./connections/druid)
- [Google Analytics](./connections/google-analytics)
- [H2](./connections/h2)
- [MongoDB (version 4.2 or higher)](./connections/mongodb)
- [MySQL (version 5.7 or higher, as well as MariaDB version 10.2 or higher)](./connections/mysql)
...
- **Syncs** get updated schemas to display in the [Data Browser](/learn/getting-started/data-browser).
...
When Metabase first connects to your database, it performs a **scan** to determine the metadata of the columns in your tables and automatically assign each column a [semantic type](../data-modeling/field-types).

During the scan, Metabase also takes a sample of each table to look for URLs, JSON, encoded strings, etc. You can map table and column metadata to new values from **Admin settings** > **Data model**. Check out [editing metadata](../data-modeling/metadata-editing).
...
During the scan, Metabase also takes a sample of each table to look for URLs, JSON, encoded strings, etc. You can map table and column metadata to new values from **Admin settings** > **Data model**. For more on editing metadata, check out [the Data Model page: editing metadata](../data-modeling/metadata-editing).
...
Metabase syncs and scans regularly, but if the database administrator has just changed the database schema, or if a lot of data is added automatically at specific times, you may want to write a script that uses the [Metabase API](/learn/administration/metabase-api) to force a sync or scan. [Our API](../api-documentation) provides two ways to initiate a sync or scan of a database:
...
2. Using an API key: `/api/notify/db/:id`. We created this endpoint so that people could notify their Metabase to sync after an [ETL operation](/learn/analytics/etl-landscape) finishes. To use this endpoint, you must pass an API key by defining the `MB_API_KEY` environment variable.
