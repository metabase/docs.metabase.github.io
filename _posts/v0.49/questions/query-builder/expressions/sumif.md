...
category: Questions
title: SumIf
source_url: 'https://github.com/metabase/metabase/blob/master/docs/questions/query-builder/expressions/sumif.md'
layout: new-docs
---
...
> [Aggregation formulas](../expressions-list#aggregations) like `sumif` should be added to the query builder's [**Summarize** menu](../../query-builder/introduction#summarizing-and-grouping-by) > **Custom Expression** (scroll down in the menu if needed).

## Parameters

- `column` can be the name of a numeric column, or a [function](../expressions-list#functions) that returns a numeric column.
- `condition` is a [function](../expressions-list#functions) or [conditional statement](../expressions#conditional-operators) that returns a boolean value (`true` or `false`), like the conditional statement `[Payment] > 100`.
...
2. Add a [**Group by**](../../query-builder/introduction#summarizing-and-grouping-by) column in the query builder.
...
| [Data type](/learn/databases/data-types-overview#examples-of-data-types) | Works with `SumIf`        |
| ------------------------------------------------------------------------------------------------ | ------------------------- |
| String                                                                                           | ❌                        |
| Number                                                                                           | ✅                        |
| Timestamp                                                                                        | ❌                        |
...
You can combine [`Sum`](../expressions-list#sum) and [`case`](./case):
...
The `GROUP BY` part of the SQL query maps to a Metabase [**Group by**](../../query-builder/introduction#summarizing-and-grouping-by) column set to "Date Received: Month".
...
These steps will produce the same result as the Metabase `SumIf` expression (with the [**Group by**](../../query-builder/introduction#summarizing-and-grouping-by) column set to "Date Received: Month").
...
- [Custom expressions tutorial](/learn/questions/custom-expressions)
