...
- Use the same `analytics` role for other BI tools in your [data stack](/learn/databases/data-landscape#data-analysis-layer) that need read-only access to the analytics tables in your database.
...
[Actions](../actions/introduction) let Metabase write back to specific tables in your database.
...
[Model caching](../data-modeling/models#model-caching) lets Metabase save query results to a specific schema in your database. Metabase's database user will need the `CREATE` privilege to set up the dedicated schema for model caching, as well as write access (`INSERT`, `UPDATE`, `DELETE`) to that schema.
...
If you're setting up multi-tenant permissions for customers who need SQL access, you can [create one database connection per customer](/learn/permissions/multi-tenant-permissions#granting-customers-native-sql-access-to-their-schema). That means each customer will connect to the database using their own database user.
