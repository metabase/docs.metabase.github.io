...
layout: new-docs
redirect_from:
    - /docs/v0.47/embedding/signed-embedding
---
...
You can can't use static embeds with [data sandboxes](../permissions/data-sandboxes), [drill-through](/learn/questions/drill-through) or [auditing tools](../usage-and-performance-tools/audit) because signed JWTs don't create user sessions (server-side sessions). For those features, check out [interactive embedding](./interactive-embedding).

To restrict data in static embeds for specific people or groups, set up [locked parameters](./static-embedding-parameters#restricting-data-in-a-static-embed) instead.
...
If you want to embed charts with additional interactive features, like [drill-down](/learn/questions/drill-through) and [self-service querying](../questions/query-builder/introduction), see [Interactive embedding](./interactive-embedding).
...
5. Optional: [Add parameters to the embed](./static-embedding-parameters).
6. Click **Publish**.
...
3. Change the [parameters](./static-embedding-parameters#customizing-the-appearance-of-a-static-embed) in your actual server code.
...
The banner appears on signed embeds created with Metabase's open-source version. You'll need to upgrade to a [Pro or Enterprise plan](/pricing) to remove the banner.
