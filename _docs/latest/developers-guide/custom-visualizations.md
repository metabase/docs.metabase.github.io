...
source_url: >-
  https://github.com/metabase/metabase/blob/master/docs/developers-guide/custom-visualizations.md
layout: new-docs
summary: >-
  Use the Custom Visualizations SDK to build, develop, and package your own
  chart types for Metabase.
latest: true
---
...
You scaffold a project with the `@metabase/custom-viz` package, write your visualization, and package it into a `.tgz` bundle. An admin uploads the plugin to Metabase (see [Custom visualizations](../questions/visualizations/custom)), and you're in business.
...
- A Metabase on a [Pro or Enterprise plan](/pricing/) to load your plugin into.
...
`series` is an array of result sets, with one entry per series on the chart. A single question produces one entry; a dashboard card with [multiple series](../dashboards/multiple-series) produces several entries. Each entry has a `data` object:
...
To match Metabase's look (and follow [dark mode](../people-and-groups/account-settings#theme)), you have two paths. For anything you render as DOM or SVG, you can style with Metabase's CSS variables: `var(--mb-color-brand)` and the other `--mb-color-*` variables, and the theme follows automatically.
...
For uploading and managing plugins, see [Custom visualizations](../questions/visualizations/custom).
...
Custom visualizations only render in the live, interactive app. Static renders, like dashboard subscriptions sent by [email](../dashboards/subscriptions), Slack, or webhook, fall back to a table for any card that uses a custom visualization. The same goes for [embedded](../embedding/introduction) questions and dashboards: a card that uses a custom visualization falls back to a table.
...
- [Visualization overview](../questions/visualizations/visualizing-results)
