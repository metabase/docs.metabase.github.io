...
> AI features are available on [Metabase Cloud](/features/metabot-ai) and on self-hosted Metabase, using either the Metabase AI service or your own AI provider API key.
...
When your connection is active, the provider card header shows **Connected to [provider]** (for example, "Connected to Anthropic") next to a green status dot. With your key connected, you get access to [Metabot](./metabot), [inline SQL generation](./metabot#inline-sql-editing), the [MCP server](./mcp), and the [Agent API](./agent-api).
...
Use the **Agent API** toggle to turn external access to the [Agent API](./agent-api) on or off.
...
This data may be included when you [submit feedback](./metabot#giving-feedback-on-metabot-responses).
