...
source_url: >-
  https://github.com/metabase/metabase/blob/master/docs/people-and-groups/oidc-keycloak.md
layout: new-docs
summary: >-
  Step-by-step guide to setting up Keycloak as an OIDC identity provider for
  Metabase.
latest: true
---
...
Keycloak is an open-source identity provider that supports [OpenID Connect (OIDC) authentication](./authenticating-with-oidc) out of the box.

1. [Set up OIDC in Keycloak](#set-up-oidc-in-keycloak) (the identity provider).
2. [Set up OIDC in Metabase](./authenticating-with-oidc#set-up-oidc-in-metabase) (the service provider).
...
If you're self-hosting Metabase, make sure you've set `MB_ENCRYPTION_SECRET_KEY` before enabling OIDC. See [OIDC-based authentication](./authenticating-with-oidc#self-hosted-metabases-must-set-an-encryption-key) for details.
...
See [OIDC-based authentication](./authenticating-with-oidc#set-up-oidc-in-metabase) for more on the Issuer URI (including container/Docker networking considerations).
...
Keycloak's default OIDC claims (`email`, `given_name`, `family_name`) match Metabase's defaults, so no extra configuration is needed. See [Attribute mapping](./authenticating-with-oidc#attribute-mapping).
...
Once Keycloak includes the groups claim in the token, set up the mapping in Metabase. See [Configure group sync in Metabase](./authenticating-with-oidc#configure-group-sync-in-metabase) for the steps.
