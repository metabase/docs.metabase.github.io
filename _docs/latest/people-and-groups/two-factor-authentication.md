...
source_url: >-
  https://github.com/metabase/metabase/blob/master/docs/people-and-groups/two-factor-authentication.md
layout: new-docs
summary: >-
  Add a second step to logins by having people confirm their identity with an
  authenticator app. A Pro/Enterprise feature.
latest: true
---
...
If you configure Metabase through environment variables or a [config file](../configuring-metabase/config-file), the matching setting is [`MB_MFA_ENFORCEMENT`](../configuring-metabase/environment-variables#mb_mfa_enforcement). Set `MB_MFA_ENFORCEMENT` to `optional` to let people enroll, or `off` to turn 2FA off.
...
Set the [`MB_ENCRYPTION_SECRET_KEY`](../databases/encrypting-details-at-rest) environment variable so Metabase encrypts authenticator secrets at rest. If you turn on 2FA without it, Metabase shows a warning on the settings page.

If you're using Metabase Cloud, we've encrypted your keys for you.

### Set up email for the fallback code

Metabase requires people to use an email address as a login, but that doesn't mean that email is set up. The email fallback only appears if your Metabase can send [email](../configuring-metabase/email). If you skip setting up email, people will have to rely on their authenticator app and recovery codes only.
...
- [Enrolling in two-factor authentication](./account-settings#two-factor-authentication)
- [Authentication options](./start#authentication)
