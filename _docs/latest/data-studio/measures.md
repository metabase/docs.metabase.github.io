...
title: Measures
source_url: 'https://github.com/metabase/metabase/blob/master/docs/data-studio/measures.md'
layout: new-docs
summary: >-
  Create saved aggregations on tables so people can use consistent calculations
...
People will see measures as options in the Summarize block of the [query builder](../questions/query-builder/editor).

![Measure in the query builder](./images/measure.png)

## Create measures

> If your instance is in read-only [remote sync](../installation-and-operation/remote-sync) mode, and you enabled sync of the [Library](./library), you will not be able to create measures on tables published in the Library while in read-only mode.

Measures are created in [Data Studio](../data-studio/overview).
...
5. Add your aggregation formula. For more complex aggregations, check out [custom expressions](../questions/query-builder/expressions).
...
You can explore measures along dimensions and compare several measures in the [Metrics Explorer](../questions/metrics-explorer).

![Explore a measure](./images/explore-measure.png)

To see all measures on a table, select the table in [Data Studio > Managing tables](./managing-tables) and switch to the **Measures** tab.

## Permissions for measures

To **create, edit, or delete** measures, people need to:

- Be admins or members of the [Data Analysts](../people-and-groups/managing#data-analysts) group. This is needed to access Data Studio.
- Have [Create queries](../permissions/data#create-queries-permissions) permissions on the measure's table. This is needed to create the measure's query.

To **use** a measure, people need to:

- Have [Create queries](../permissions/data#create-queries-permissions) permissions on the measure's table.

## Only measures on published tables are synced to Git

When using [Remote Sync](../installation-and-operation/remote-sync):

- Measures on tables that are _not_ published to the [Library](./library) are _not_ synced;
- Measures on published tables are only synced if [_Library sync is enabled_](./library#versioning-the-library).
...
You can, however, sync [Metrics](../data-modeling/metrics) on any tables by syncing their collection.

## Limitations

- Measures are only available for use in the query builder. For defining reusable bits of SQL, check out [Snippets](../questions/native-editor/snippets).
