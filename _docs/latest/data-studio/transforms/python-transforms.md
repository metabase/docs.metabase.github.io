...
source_url: >-
  https://github.com/metabase/metabase/blob/master/docs/data-studio/transforms/python-transforms.md
layout: new-docs
summary: >-
  Use Python to wrangle your data in Metabase and write the results back to your
  database.
latest: true
---
...
- If you're self-hosting Metabase, you'll need to:
  ...
  2.  Set up a self-hosted Python runner, see [Python runner](python-runner).
...
3. Select a database that has the data you want to transform. See [Databases that support transforms](transforms-overview#databases-that-support-transforms).
...
10. Optionally, assign tags to your transforms. Tags are used by [jobs](jobs-and-runs) to run transforms on schedule.
...
See [Run a transform](transforms-overview#run-a-transform). You'll see logs for a transform run (including the output of any `print()` statements you included in the `transform()` code) on the transform's page.
...
To update a table incrementally, your data has to have a certain structure. See [Prerequisites for incremental transforms](./transforms-overview#prerequisites-for-incremental-transforms).
...
- Because Python transforms use `pandas`, all data manipulation is done in memory. The available memory is determined by the Python execution add-on. For large datasets, consider using [query-based transforms](./query-transforms) that run in your database.
