...
3. Write your transform query as you would normally write a query in Metabase. See [Query builder](../../questions/query-builder/editor) and [SQL editor](../../questions/native-editor/writing-sql) documentation for more information.
...
6. Optionally, assign tags to your transforms. Tags are used by [jobs](jobs-and-runs) to run transforms on schedule.
...
The incremental `{%raw%}[[WHERE id > {{checkpoint}}]]{% endraw %}` pattern shown in [Incremental query transforms](#incremental-query-transforms) is an example of this an optional variable in practice. See also [optional variables](../../questions/native-editor/optional-variables).
