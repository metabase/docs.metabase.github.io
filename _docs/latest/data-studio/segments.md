...
title: Segments
source_url: 'https://github.com/metabase/metabase/blob/master/docs/data-studio/segments.md'
layout: new-docs
summary: >-
  Create saved filters on tables so people can use consistent definitions when
  building queries.
latest: true
---
...
To see all segments on a table, select the table in [Managing tables](./managing-tables) and switch to the **Segments** tab.

## Create segments

> If your instance is in read-only [remote sync](../installation-and-operation/remote-sync) mode, and you enabled sync of the [Library](./library), you will not be able to create segments on tables published in the Library while in read-only mode.

Segments are created in [Data Studio](../data-studio/overview).
...
- Have [Create queries](../permissions/data#create-queries-permissions) permissions on the segment's table. This is needed to create the segment's query.

To **use** a segment, people need to:

- Have [Create queries](../permissions/data#create-queries-permissions) permissions on the segment's table.

## Only segments on published tables are synced to Git

When using [Remote Sync](../installation-and-operation/remote-sync):

- Segments on tables that are _not_ published to the [Library](./library) are _not_ synced;
- Segments on published tables are only synced if [_Library sync is enabled_](./library#versioning-the-library).
...
- Segments are only available for use in the query builder. For defining reusable bits of SQL, check out [Snippets](../questions/native-editor/snippets).
