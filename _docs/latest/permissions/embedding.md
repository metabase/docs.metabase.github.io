...
source_url: 'https://github.com/metabase/metabase/blob/master/docs/permissions/embedding.md'
layout: new-docs
summary: >-
  Learn about which permissions tooling you should use depending on whether your
  customer data is in one database or split across multiple databases.
latest: true
---
...
- [Data sandboxing](./data-sandboxes) to restrict rows and columns.
...
To display a filtered version of **Data** to different tenants based on a `Tenant_ID`, you can create a [basic sandbox](./data-sandboxes#types-of-data-sandboxes).
...
3. **Sandbox the table** for the group to apply the [row-level security based on user attributes](./data-sandboxes#types-of-data-sandboxes).
...
To keep A and C from viewing the `Insights` column, you can create a [custom sandbox](./data-sandboxes#types-of-data-sandboxes) to restrict both the rows and columns they see when they view the table.
...
6. [Create a custom sandbox](./data-sandboxes#types-of-data-sandboxes) using the "Metrics-Only Tenants" group and "Customer Metrics" SQL question.
...
For more fine-grained control over what individuals can see, even within the same tenants, you can also use the other tools Metabase provides, like [data sandboxing](./data-sandboxes) and [connection impersonation](./impersonation), in combination with database routing.
