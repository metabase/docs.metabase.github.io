...
category: Embedding
title: Tenants
source_url: 'https://github.com/metabase/metabase/blob/master/docs/embedding/tenants.md'
layout: new-docs
summary: >-
  Tenants let you serve multiple customers from a single Metabase while keeping
  their data isolated. Configure tenant users, groups, and shared collections,
  then use row-level security or impersonation to ensure each tenant only sees
  their own data.
latest: true
---
...
Tenant users and and internal users can be organized into Metabase [groups](../people-and-groups/managing#groups).
...
Once you add a tenant attribute, all users of that tenant will inherit the attribute, but the value can overridden for any particular user, see [Edit user attributes](../people-and-groups/managing#adding-a-user-attribute).

Currently, you can't assign custom tenant attributes with SSO. The only way to assign attributes is through the Metabase UI (but you can provision attributes for _individual users_ through SSO, see [JWT user attributes](../people-and-groups/authenticating-with-jwt)). However, if you're using user attribute to set permissions, then you can use the [special slug attribute](#special-tenant-slug-attribute) which Metabase creates automatically.
