...
How you authenticate the embed determines how cool the embed can get. This page covers the SSO setup, which lets you do everything. This setup requires you to have [set up SSO for your Metabase](./authentication). You can check out a [comparison between SSO and guest](./introduction#comparison-of-embedding-types), or jump straight to the [guest embed docs](./guest-embedding). But we recommend setting up modular embedding with SSO.
...
For production use, you'll need to set up SSO authentication. See [Modular embedding - authentication](./authentication) for details on setting up JWT or SAML authentication.
