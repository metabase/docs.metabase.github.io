...
source_url: 'https://github.com/metabase/metabase/blob/master/docs/embedding/appearance.md'
layout: new-docs
summary: >-
  Style your embedded Metabase components using themes to customize colors,
...
To use dark mode, set `theme.preset` to `"dark"` in the `defineMetabaseConfig()` function of your [embedding code snippet](./modular-embedding#add-the-embedding-script-to-your-app):
...
When you create a new embed using the [embed wizard](./modular-embedding#3-customize-your-embed), the last customization step lets you pick:
...
If you're using the [SDK](./sdk/introduction), pass a theme to `MetabaseProvider` with `defineMetabaseTheme`, which accepts the same theme shape that you configure in the Themes admin UI. You can copy a saved theme's values into your code:
