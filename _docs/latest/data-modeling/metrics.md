...
summary: >-
  Metrics are reusable, standardized aggregations that let your team
  consistently define important numbers across your Metabase.
latest: true
---
...
Metrics live in collections. You can save metrics into the [Library](../data-studio/library) collection to surface your org's official curated metrics and encourage people to use them for their own questions.
...
1. Bring up the command palette with cmd/ctrl + k (that’s cmd + k for Macs, or ctrl + k for Windows PCs) and search for **New metric**
   ...
   If you're creating a [Library metric](../data-studio/library#metrics), you can also [navigate to the Library](../data-studio/library) and click **+ New > Metric**.
...
3. Define your metric. The metric editor is similar to the regular query builder, with two key differences:
   ...
   - The [**Formula**](#metric-formula) block is where you define your aggregation. You can use [custom expressions](../questions/query-builder/expressions) to write the formula, for example `SumIf([Total], [Plan] != 'free')`.
...
To break metrics out by different dimensions, compare metrics and [measures](../data-studio/measures), and do math with metrics (like add or divide metrics by each other), check out the [Metrics Explorer](../questions/metrics-explorer).
...
We don't recommend using joins in metric definitions. If you need to build a metric that uses information from multiple tables, consider making a [transform](../data-studio/transforms/transforms-overview) to join those tables, _then_ create a metric on the table the transform creates.

## Metric formula

The formula is the core of the metric. It's the thing you are aggregating, and it's required when defining a metric.

You can use [custom expressions](../questions/query-builder/expressions) to define a metric, for example:

```
 (CountIf([Score] >= 9) - CountIf([Score] <= 6) ) / Count() * 100
```

A metric formula must be an _aggregation_ of a column, meaning you can have a formula like `Sum([Price])` which will add up all the values in the `Price` column across all records, but you can't have a formula like `[Price] + [Tax]` which adds two values on a per-record basis. If you want to create a "calculated" column, consider creating a [transform](../data-studio/transforms/transforms-overview) or [model](../data-modeling/models) instead.
...
Because metrics live in collections, are attached to a data source, and define a query, permissions for metrics are determined by a combination of [collection permissions](../permissions/collections), ["View data" permissions](../permissions/data#can-view-data-permission), and ["Create queries" permissions](../permissions/data#create-queries-permissions).
...
By adding metrics to your Library, the query builder will display the metrics more prominently in menus. See [Library metrics](../data-studio/library#metrics).
...
- Metrics are only available to the query builder. For defining reusable bits of SQL, check out [Snippets](../questions/native-editor/snippets).
