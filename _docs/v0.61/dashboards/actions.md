...
source_url: 'https://github.com/metabase/metabase/blob/master/docs/dashboards/actions.md'
layout: new-docs
---

# Actions on dashboards

You can add buttons to dashboards to perform [actions](../actions/start) like creating or updating records, and combine them with dashboard [filter](./filters).

![Dashboard with filter, action button, and detail card view](./images/dashboard-filter-action.png)

## Create an action

To add an action button to a dashboard, you'll first need to create an action on a model. See [Model actions](../actions/introduction).

## Add an action button to a dashboard

Once you [created an action](../actions/introduction), you can add a button to a dashboard to perform that action:
...
2. [Add a filter](./filters) to a dashboard, connect the filter to one or more cards, and click **Done** in the bottom of the sidebar.
