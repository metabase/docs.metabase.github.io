...
layout: new-docs
redirect_from:
    - /docs/master/users-guide/sql-snippets
    - /docs/master/questions/native-editor/sql-snippets
---
...
2. Write some SQL or native code and highlight a section of code that you want to save to reuse later. The section doesn't have to be a whole query. For example, you can highlight:
   ...
   - [references to saved questions or models](./referencing-saved-questions-in-queries), like `{% raw %}{{#123-orders-model}}{% endraw %}`.
...
## Values for snippet parameters are defined by the question, not the snippet
...
You can also use [table variables](./table-variables) in snippets to write a generic query once and reuse it across different tables, inserting a new table in each question.
