...
layout: new-docs
redirect_from:
    - /docs/master/administration-guide/whitelabeling
    - /docs/master/enterprise-guide/whitelabeling
    - /docs/master/enterprise-guide/customize-embeds
    - /docs/master/embedding/whitelabeling
    - /docs/master/embedding/fonts
    - /docs/master/embedding/customize-embeds
...
When choosing interface color, keep in mind that some users of your Metabase can choose to display the interface in [dark mode](../people-and-groups/account-settings#theme). Pick colors that work well both in light and dark mode.
...
- [Pin and grid maps](../questions/visualizations/map)

Only a **limited subset** of custom colors will be used for:

- Conditional formatting in [tables](../questions/visualizations/table) and [pivot tables](../questions/visualizations/pivot-table)
...
Keep in mind that if you hide documentation links, people using your Metabase might not be able to troubleshoot their problems or learn how to use Metabase features like [custom expressions](../questions/query-builder/expressions).

### Help link in the settings menu

The Settings menu (the "grid" menu in the upper right of your Metabase) includes a **Help** option that links to a [Metabase help page](/help/) by default. You can change this menu Help item by selecting one of the following options:
