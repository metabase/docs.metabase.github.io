...
layout: new-docs
redirect_from:
    - /docs/master/administration-guide/03-metadata-editing
---
...
Descriptions are displayed in Metabase's [data reference](../exploration-and-organization/data-model-reference) to help people find the right table for their use case.
...
To change the _global_ display name of a column in Metabase, click on the name of the column. For example, you could display "auth.user" as "User" to make the column more readable. People can also use [models](./models) to give columns a display name that's local to the model.
...
You can change the [semantic type](../data-modeling/semantic-types) to give people more context. For example, you could set an Integer as a "Score" so people have a better idea what those integers indicate.
...
The **Filtering** setting changes a column's [filter widget](../dashboards/filters). Options include:
