...
source_url: 'https://github.com/metabase/metabase/blob/master/docs/embedding/full-app-embedding-quick-start-guide.md'
layout: new-docs
redirect_from:
    - /docs/master/embedding/interactive-embedding-quickstart
    - /docs/master/embedding/interactive-embedding-quickstart-guide
    - /docs/master/embedding/interactive-embedding-quick-start-guide
---
...
You could also use the dashboard's [Entity ID](../installation-and-operation/serialization#metabase-uses-entity-ids-to-identify-metabase-items). On the dashboard, click on the **info** button. On the **Overview** tab, look for the dashboard's **Entity ID**. Copy that Entity ID. You'll use that Entity ID in the iframe's `src` URL: (e.g., `src=/dashboard/entity/[Entity ID]`).
