...
title: 'Remote sync'
source_url: 'https://github.com/metabase/metabase/blob/master/docs/installation-and-operation/remote-sync.md'
layout: new-docs
...
Metabase doesn't sync any of your data. What it stores in Git are [YAML files](./serialization#example-of-a-serialized-question) describing your analytics content. Your actual data stays in your databases and never goes to GitHub.
...
**You choose what to sync**: You can sync the Library, any top-level collections, and transforms. Everything inside selected collections (including sub-collections) is versioned and synchronized with your repository. If you use [Tenants](../embedding/tenants), you can also sync shared collections.
...
6. (Optional) If you have [multi-tenant user strategy enabled](../embedding/tenants#enable-multi-tenant-strategy), you can also choose which [shared collection](../embedding/tenants#changing-tenant-strategy) to sync.
...
If you use [tenants](../embedding/tenants), you can also choose to sync [shared collections](../embedding/tenants#collection-types).
...
To version control your data transformation logic, you can sync your [Transforms](../data-modeling/transforms) including all your tags and jobs. Transform syncing is all or nothing: Metabase will sync your entire transforms namespace. You can't selectively sync specific transform folders.
...
If you are using [tenants](../embedding/tenants), you can also sync shared collections. You'll see the same yellow dot indicator for shared collections.
...
Sometimes you might want synced content to appear at the top level of your navigation rather than nested under a synced collection. If you use [Tenants](../embedding/tenants), you can accomplish that with synced shared collections.
...
Remote Sync serializes your Metabase content as YAML files in your repo. YAML files use Metabase [serialization format](./serialization). See [Serialization docs](./serialization) for more information on the format.
...
Remote Sync doesn't sync table metadata, so if you're importing and exporting your [table metadata](../data-modeling/metadata-editing), you should stick with serialization.
