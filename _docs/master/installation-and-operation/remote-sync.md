...
Remote Sync uses the same underlying serialization format as the [Metabase CLI serialization feature](./serialization), but serves a different purpose:
...
For more details on the serialization format and command-line workflows, see the [serialization documentation](./serialization).
