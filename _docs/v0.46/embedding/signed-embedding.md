...
source_url: 'https://github.com/metabase/metabase/blob/master/docs/embedding/signed-embedding.md'
layout: new-docs
---
...
Signed embeds can't be used with [data sandboxes](../permissions/data-sandboxes) or [auditing tools](../usage-and-performance-tools/audit) because signed JWTs don't create user sessions (server-side sessions).

To restrict data in signed embeds for specific people or groups, set up [locked parameters](./signed-embedding-parameters#restricting-data-in-a-signed-embed) instead.
...
If you want to embed charts with additional interactive features, like [drill-down](/learn/questions/drill-through) and [self-service querying](../questions/query-builder/introduction), see [Full-app embedding](./full-app-embedding).
