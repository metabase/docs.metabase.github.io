...
redirect_from:
    - /docs/v0.58/enterprise-guide/authenticating-with-jwt
---
...
- **Tenant attribute:** the key to retrieve each JWT user's tenant. Default is `tenant`. See [Assigning tenant users to tenants](./assigning-users-to-tenants).
...
If you're running a multi-tenant application, you can use JWT to automatically assign users to tenants based on a claim in the JWT token. See [Assigning tenant users to tenants](./assigning-users-to-tenants) for details.
