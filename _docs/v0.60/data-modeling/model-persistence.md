...
source_url: 'https://github.com/metabase/metabase/blob/master/docs/data-modeling/model-persistence.md'
layout: new-docs
---

# Model persistence

> Prefer [Transforms](../data-studio/transforms/transforms-overview) instead of model persistence. Model persistence remains supported for now, but it will be deprecated in future versions of Metabase. You can [convert models to transforms in bulk](../data-studio/transforms/transforms-overview#convert-models-to-transforms).
...
The cron scheduler uses the [default instance report Timezone](../configuring-metabase/localization#set-default-instance-report-timezone) if selected. Otherwise the scheduler will use the System Timezone (which defaults to GMT in [Metabase Cloud](/cloud/)).
