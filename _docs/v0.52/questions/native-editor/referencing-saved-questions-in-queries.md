...
layout: new-docs
redirect_from:
    - /docs/v0.52/users-guide/referencing-saved-questions-in-queries
---
...
The same syntax can be used in [Common Table Expressions (CTEs)](/learn/grow-your-data-skills/learn-sql/working-with-sql/sql-cte) (with SQL databases that support CTEs):
...
- You cannot refer to variables in sub-queries. You only have access to the _results_ of the model or saved question, not the model or saved question's query. For example, if you have a saved question that uses a [field filter](/learn/metabase-basics/querying-and-dashboards/sql-in-metabase/field-filters), you won't be able to reference that variable. If you need to change how the saved question has filtered the results, you'll need to update (or duplicate) that question and apply the filter.

## Further reading

- [Models](../../data-modeling/models)
- [SQL Snippets](/learn/metabase-basics/querying-and-dashboards/sql-in-metabase/sql-snippets)
- [SQL Snippets vs Saved Questions vs. Views](/learn/metabase-basics/querying-and-dashboards/sql-in-metabase/organizing-sql)
