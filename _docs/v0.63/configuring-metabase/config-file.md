...
- The path specified by the [`MB_CONFIG_FILE_PATH`](./environment-variables#mb_config_file_path) environment variable.
...
Since loading from a config file is a Pro/Enterprise feature: for new installations, you'll need to supply Metabase with a token using the [`MB_PREMIUM_EMBEDDING_TOKEN`](./environment-variables#mb_premium_embedding_token) environment variable.
