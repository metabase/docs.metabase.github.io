...
layout: new-docs
redirect_from:
    - /docs/v0.48/enterprise-guide/data-sandboxes
---
...
Or, if your customers want to log into your Metabase themselves for [self-service analytics](/learn/customer-facing-analytics/multi-tenant-self-service-analytics), you can use a sandbox to make sure each customer can only view the rows that match their customer ID.
...
If you're ready to jump in, try our [Data sandbox examples](./data-sandbox-examples).
...
Data sandboxes show specific data to each person based on their [user attributes](../people-and-groups/managing#adding-a-user-attribute). You can:
...
- [Pass a user attribute to a Markdown parameter](/learn/dashboards/markdown#custom-url-with-a-sandboxing-attribute).
...
[Native query permissions](../permissions/data) will get automatically **disabled** for any groups that you add to a data sandbox. If you need to prevent people with SQL access from querying specific tables in a database, you can edit your [database users, roles, and privileges](../databases/users-roles-privileges).
...
- A [group](../people-and-groups/managing#groups) of people to be added to the basic sandbox.
...
**User attributes are required for basic sandboxes** and optional for custom sandboxes. When [adding a new user attribute](../people-and-groups/managing#adding-a-user-attribute), you'll set up a key-value pair for each person.
...
- [Restricting rows in basic sandboxes](./data-sandbox-examples#basic-sandbox-setup)
- [Restricting rows in custom sandboxes](./data-sandbox-examples#example-2-using-variables-in-a-saved-question)
- [Displaying custom text in Markdown dashboard cards](/learn/dashboards/markdown#custom-url-with-a-sandboxing-attribute)
...
You can find a sample basic sandbox setup in the [Data sandbox examples](./data-sandbox-examples).

## Prerequisites for custom sandboxes

- A [group](../people-and-groups/managing#groups) of people to be added to the advanced data sandbox.
...
- A [saved SQL question](../questions/native-editor/writing-sql) with the rows and columns to be displayed to the people in the custom sandbox, stored in the admin-only collection.
...
> Make sure to save the SQL question in an admin-only collection ([collection permissions](../permissions/collections) set to **No access** for all groups except Administrators). For more info, see [Permissions conflicts: saved SQL questions](#saved-sql-questions).
...
You can find sample custom sandbox setups in the [Data sandbox examples](./data-sandbox-examples).
...
For a sample SQL variable and user attribute setup, see the [Data sandbox examples](./data-sandbox-examples).
...
In step 2 of the [row restriction setup](#restricting-rows-in-an-custom-sandbox-with-user-attributes) above, you'll add a SQL variable so that the `WHERE` clause will accept a dynamic value. The [SQL variable type](../questions/native-editor/sql-parameters#sql-variable-types) must be text, number, or date:
...
To prevent this from happening, you'll have to [disable public sharing](../questions/sharing/public-links) for your Metabase instance.
...
- [Permissions strategies](/learn/permissions/strategy)
- [Configuring permissions for different customer schemas](/learn/permissions/multi-tenant-permissions)
- [Securing embedded Metabase](/learn/customer-facing-analytics/securing-embeds)
