...
layout: new-docs
redirect_from:
    - /docs/v0.48/administration-guide/14-caching
    - /docs/v0.48/enterprise-guide/cache
---
...
If your questions share a common model, you can enable [model caching](../data-modeling/models#model-caching) instead.

## Caching doesn't work with data sandboxing

Just something to keep in mind: if someone in a [sandboxed](../permissions/data-sandboxes) group views a table, Metabase will skip the cached results and query the table directly, returning only the results the person is allowed to see.
...
On [paid plans](/pricing/), you can view statistics about query execution time from your [auditing tools](../usage-and-performance-tools/audit).
...
All Metabase editions include global caching controls. On [paid plans](/pricing/), you can override your global [time-to-live (TTL) setting](#cache-time-to-live-ttl-multiplier) to set different cache lifetimes for specific databases, questions, or dashboards.
