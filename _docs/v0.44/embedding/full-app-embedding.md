...
layout: new-docs
redirect_from:
    - /docs/v0.44/enterprise-guide/full-app-embedding
---
...
Metabase offers several [types of embedding](./introduction) with different levels of customization and security.
...
If you only want to set up a fixed number of filters and drill-down views into your data (i.e., prevent people from creating their own [questions](/glossary/question)), you might prefer [Signed embedding](./signed-embedding).
...
For example, to embed your Metabase home page, set the `src` attribute to your [site URL](../configuring-metabase/settings#site-url).
...
If you want to embed Metabase in another domain (e.g., Metabase is hosted at `metabase.yourcompany.com`, but you want to embed Metabase at `yourcompany.github.io`), set the following [environment variable](../configuring-metabase/environment-variables):
