...
layout: new-docs
redirect_from:
    - /docs/v0.44/administration-guide/03-metadata-editing
---
...
This page focuses on editing table and column metadata; another page covers [segments and metrics](./segments-and-metrics).

Or are you instead looking for docs on [models](./models)?
...
Visibility settings are distinct from **permissions**. Users can still query hidden tables using the **SQL editor**. See [**permissions**](../permissions/start) for controlling access to data.
...
You can use the Data Model page to edit field types for fields in your database. Use the **Type** dropdown menu to select from this [list of options](./field-types).

In this same menu, you can also designate a column as the table's **primary key** or **foreign key**.

See [Field types](./field-types) for more on types and how they function in Metabase.
