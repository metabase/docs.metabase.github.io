...
You can also select multiple tables from the same database by [joining them](./join.html).
...
If your Metabase administrators have created special named filters for the table you're viewing, they’ll appear at the top of the filter dropdown in purple text with a star next to them. These are called [**Segments**](../administration-guide/07-segments-and-metrics.html), and they're shortcuts to a combination of filters that are commonly used in your organization. They might be called things like “Active Users,” or “Most Popular Products.”

### Filters with `OR`

![Filter expression](./images/expressions/filter-expression.png)

If you have a more complex filter you're trying to express, you can pick **Custom Expression** from the add filter menu to create a filter expression. You can use comparison operators like greater than, `>`, or less than ,`<`, as well as spreadsheet-like functions. For example, `[Subtotal] > 100 OR median([Age]) < 40`. Learn more about writing [expressions](./expressions.html) or skip right to the [list of expressions](expressions-list.html).
...
Next, we'll learn how to [visualize results](05-visualizing-results.html).
