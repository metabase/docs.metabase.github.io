...
Run the migration command, `load-from-h2`, using the appropriate [environment variables](environment-variables) for the target database you want to migrate to.
...
From the directory with your H2 file and your Metabase JAR, run the migration command, `load-from-h2`. Use the appropriate connection string or [environment variables](environment-variables) for the target database you want to migrate to. The command would look something like:
...
See [Running Metabase database migrations manually](running-migrations-manually).
