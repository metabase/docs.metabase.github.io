...
If you'd like to have your users authenticate with SAML or JWT, Metabase's [paid plans](/pricing) let you do just that. Learn more about authenticating with [SAML](../enterprise-guide/authenticating-with-saml) or [JWT](../enterprise-guide/authenticating-with-jwt).
...
Manually assigning people to [groups](05-setting-permissions#groups) in Metabase after they've logged in via SSO can get tedious. Instead, you can take advantage of the groups that already exist in your LDAP directory by enabling [group mappings](/learn/permissions/ldap-auth-access-control#group-management).
...
For a tutorial on setting up LDAP in Metabase, check out this [Learn lesson](/learn/permissions/ldap-auth-access-control). If you run into an issue, our [LDAP troubleshooting guide](../troubleshooting-guide/ldap) can help.
...
If you're running a [paid version of Metabase](/pricing) and using [data sandboxes](../enterprise-guide/data-sandboxes), you can use existing LDAP [user attributes](../enterprise-guide/data-sandboxes#getting-user-attributes) when granting sandboxed access.
